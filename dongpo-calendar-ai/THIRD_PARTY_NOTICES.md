# 第三方字体与音乐授权说明

本文件用于开发、上线与内容审查。正式发布时，关于页、项目内许可证与实际使用文件必须保持一致。

## 1. 志莽行（Zhi Mang Xing）

- 字体名称：志莽行（Zhi Mang Xing）
- 上游字体文件：`ZhiMangXing-Regular.ttf`（未纳入当前原型）
- 项目作者：The Zhi Mang Xing Project Authors
- 版权声明：Copyright 2018 The Zhi Mang Xing Project Authors
- 上游项目：https://github.com/google/fonts/tree/main/ofl/zhimangxing
- 许可协议：SIL Open Font License 1.1
- 本地许可证：`fonts/OFL-ZhiMangXing.txt`
- 本项目状态：当前原型不加载该字体文件，仅在关于页保留字体名称、作者与许可说明。
- 使用提醒：后续如正式接入，应由前端另行评估小程序包体、字符覆盖与字体加载方案。

## 2. 古琴曲《醉渔唱晚》

- 曲名：《醉渔唱晚》（Zuiyu Changwan / The Evening Song of the Drunken Fisherman）
- 乐器：古琴（尼龙弦）
- 演奏与录音：Charlie Huang（Charles R. Tsua）
- 录制时间：2013-10-06
- 原始琴谱来源：《天闻阁琴谱》（1876）
- 来源页面：https://commons.wikimedia.org/wiki/File:Guqin-Zuiyu_Changwan.ogg
- 许可协议：Creative Commons Attribution-ShareAlike 3.0 Unported（CC BY-SA 3.0）
- 许可链接：https://creativecommons.org/licenses/by-sa/3.0/
- 本项目状态：已选定为背景音乐；当前原型不加载或内置该音频文件，仅在关于页保留署名与许可说明。
- 使用提醒：发布时必须保留演奏者署名、来源和许可链接，并说明是否剪辑、转码或进行了其他改动；如形成改编作品，须按相同或兼容许可分享。

## 3. Noto CJK 回退字体

- 字体名称：Noto Sans SC、Noto Serif SC
- 上游项目：https://github.com/notofonts/noto-cjk
- 版权声明：Copyright 2014–2021 Adobe，保留字体名“Source”；Noto 为 Google LLC 商标。
- 许可协议：SIL Open Font License 1.1
- 本地许可证：`fonts/OFL-1.1.txt`
- 使用范围：正文与缺字回退。
