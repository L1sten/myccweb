...
test('cart remains accessible empty, supports removing unavailable products and uses confirmation snapshots',()=>{
 const f=fixture();f.state.role='hotel_admin';assert.match(f.storeCheckout(),/storeCart/);assert.match(f.storeCheckout(),/disabled/);const p=f.data.products[0];f.setCart({[p.id]:2});assert.match(f.storeCartRows(true),/storeRemove:/);const draft=f.storeSnapshot(),before=f.balance(draft.hotel);p.price+=100;assert.match(f.storeCommit(draft,'price-change').error,/价格已变化/);assert.equal(f.getCart()[p.id],2);assert.equal(f.balance(draft.hotel),before);p.status='INACTIVE';f.storeAction('storeRemove:'+p.id);assert.equal(f.getCart()[p.id],undefined);
});
test('confirmed store submission is atomic, checks current stock and balance, and retry cannot double charge',()=>{
 const f=fixture();f.state.role='hotel_admin';const p=f.data.products[0];p.stock=5;f.setCart({[p.id]:2});const d=f.storeSnapshot(),before=f.balance(d.hotel);p.stock=1;assert.match(f.storeCommit(d,'store-once').error,/库存不足/);assert.equal(f.getCart()[p.id],2);p.stock=5;f.data.balances[d.hotel]=0;assert.match(f.storeCommit(d,'store-once').error,/余额不足/);f.data.balances[d.hotel]=before;const result=f.storeCommit(d,'store-once');assert.ok(result.order);assert.equal(p.stock,3);assert.equal(f.balance(d.hotel),before-d.cents);assert.equal(result.order.itemLines[0].unitPrice,d.lines[0].unitPrice);assert.equal(Object.keys(f.getCart()).length,0);assert.equal(f.storeCommit(d,'store-once').order.no,result.order.no);assert.equal(p.stock,3);assert.equal(f.balance(d.hotel),before-d.cents);
});

test('platform mall hotel filter uses organization IDs, includes inactive hotels and paginates filtered totals',()=>{
 const f=fixture();f.state.role='platform_admin';const hotels=f.mallOptions();assert.ok(hotels.length>1);const a=hotels[0],b=hotels[1];f.org(b.id).name=f.org(a.id).name;f.org(b.id).active=false;const opts=f.mallOptions();assert.notEqual(opts[0].label,opts[1].label);assert.match(opts[1].label,/停用/);f.data.storeOrders=Array.from({length:25},(_,i)=>({no:'QA'+i,hotel:a.id,lines:'商品',count:1,cents:100,time:'2026-09-24'})).concat([{no:'OTHER',hotel:b.id,lines:'旧商品',count:1,cents:100,time:''}]);f.mallFilter(opts[0].label);assert.equal(f.mallRows().length,25);const first=f.mallPage();assert.match(first,/共 25 条/);assert.match(first,/下单酒店/);assert.doesNotMatch(first,/QA20/);f.mallSetPage(2);assert.match(f.mallPage(),/QA24/);f.mallFilter(opts[1].label);assert.match(f.mallPage(),/第 1 \/ 1 页/);assert.equal(f.mallRows()[0].no,'OTHER');f.mallFilter('');assert.equal(f.mallRows().length,26);f.mallFilter('不存在');assert.equal(f.mallRows().length,0);
});
