(function(){
  ...
  function actualResource(section,role){
    ...
    if(section==='客户与组织')return header(section,'查看平台全部组织，并建立包含首个作业站点的洗涤厂组织。',platformWriter?button('新增洗涤厂','createSite','primary-button'):'')+panel('组织清单','',table(['组织编号','组织名称','类型','状态'],[['org-platform','优洗通平台',badge('平台',''),'正常'],['org-hotel-yunqi','云栖酒店',badge('酒店','success'),'正常'],['org-factory-1','桐乡洗涤一厂',badge('洗涤厂','warning'),'正常']]));
    if(section==='酒店与楼层')return header(section,'酒店建档同时建立站点、楼层、零余额钱包和首个管理员。',platformWriter?button('新增酒店','createHotel','primary-button'):'')+panel('酒店档案','',table(['酒店名称','酒店站点','所属区域','地址','楼层','楼层数','状态'],[['云栖酒店','云栖酒店站点','城东片区','桐乡市振兴路 18 号','1F、2F、3F、8F','4',badge('正常','success')]]));
    ...
    if(section==='司机负责范围')return header(section,'建立司机账号并维护静态负责区域和酒店清单。',platformWriter?button('新增司机','editDriver','primary-button'):'')+panel('司机账号','',table(['司机','登录账号','所属组织','负责区域','负责酒店','分配数','状态'],[['司机张师傅','driver','桐乡洗涤一厂','城东片区','云栖酒店','1',badge('正常','success')]]));
    ...
    if(section==='用户与权限')return header(section,'查看平台账号；平台管理员与平台运营可按组织类型创建符合角色边界的账号。',platformWriter?button('新增用户','createUser','primary-button'):'')+panel('账号列表','',table(['姓名','登录账号','所属组织','组织类型','角色','状态','创建时间'],[['酒店管理员','hotel_admin','云栖酒店','酒店','酒店管理员',badge('正常','success'),'2026-09-01 09:00'],['工厂备货员','factory_operator','桐乡洗涤一厂','洗涤厂','工厂操作员',badge('正常','success'),'2026-09-01 09:00']]));
  ...
  function formRow(label,value,help){return '<label class="form-row"><span>'+esc(label)+'</span><input aria-label="'+esc(label)+'"'+(/密码/.test(label)?' type="password" minlength="8"':/（分）|库存/.test(label)?' type="number" min="0" step="1"':'')+' value="'+esc(value||'')+'">'+(help?'<small>'+esc(help)+'</small>':'')+'</label>';}
  function showConfigForm(action){
    ...
    var config={
      ...
      createHotel:{title:'新增酒店',kicker:'建立酒店档案',content:'<div class="form-grid">'+formRow('酒店名称','云栖酒店')+formRow('酒店站点名称','云栖酒店站点')+formRow('所属区域','城东片区')+formRow('酒店地址','桐乡市振兴路 18 号')+formRow('楼层名称','1F、2F、3F、8F')+formRow('管理员登录账号','hotel_admin_02')+formRow('管理员初始密码','********')+formRow('管理员姓名','酒店管理员')+'</div>'},
      ...
      editDriver:{title:'新增司机并分配首家酒店',kicker:'司机负责范围',content:'<div class="form-grid">'+formRow('雇主组织编号','org-factory-1')+formRow('登录账号','driver_02')+formRow('初始密码','********')+formRow('司机姓名','司机李师傅')+formRow('负责区域','城东片区')+formRow('首家酒店组织编号','org-hotel-yunqi')+'</div>'},
      createUser:{title:'新增组织用户',kicker:'用户与权限',content:'<div class="form-grid">'+formRow('所属组织编号','org-hotel-yunqi')+formRow('登录账号','hotel_operator_02')+formRow('初始密码','********')+formRow('姓名','酒店操作员')+formRow('角色','酒店操作员')+'</div>'},
      ...
      ,createSite:{title:'建立洗涤厂组织',kicker:'客户与组织',content:'<div class="form-grid">'+formRow('组织类型','洗涤厂')+formRow('组织名称','桐乡洗涤二厂')+formRow('首个站点名称','北区站')+formRow('覆盖区域','城北片区')+formRow('站点地址','桐乡市环城北路 10 号')+'</div>'},
  ...
  function handleClick(event){
    ...
    var section=event.target.closest('[data-section]');if(section){state.section=section.dataset.section;render();return;}
