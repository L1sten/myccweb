(function(){
  ...
  function identityFilter(isOrg,orgId){var q=isOrg?identity.orgQueryDraft||identity.orgQuery||{}:identity.queryDraft||identity.query;return '<form class="filter-bar identity-filter" data-filter-scope="'+(isOrg?'organizations':'users')+'">'+identityInput(isOrg?'组织名称':'姓名或登录账号','q',q.q,'placeholder="'+(isOrg?'搜索组织名称':'搜索姓名或登录账号')+'"')+(isOrg?identitySelect('组织类型','type',['hotel','factory','platform'].map(function(t){return{id:t,name:identityTypes[t]};}),q.type,'全部类型'):(!orgId?'<label class="form-row"><span>所属组织</span><input data-identity-field="orgSearch" list="identityFilterOrgs" aria-label="所属组织" placeholder="全部组织" value="'+esc((identityOrg(q.org)||{}).name||'')+'"><datalist id="identityFilterOrgs">'+identity.orgs.map(function(o){return '<option value="'+esc(o.name)+'"></option>';}).join('')+'</datalist></label>':'')+identitySelect('角色','role',Object.keys(identityCapabilities).map(function(r){return{id:r,name:roles[r].label};}),q.role,'全部角色')+identitySelect('账号状态','status',[{id:'enabled',name:'启用'},{id:'disabled',name:'停用'}],q.status,'全部状态'))+'<div class="filter-actions">'+button('查询','identitySearch','primary-button')+button('重置','identityReset','secondary-button')+'</div></form>';}
  function identityMemberRows(users){return users.slice().sort(function(a,b){return b.createdAt.localeCompare(a.createdAt)||a.id.localeCompare(b.id);}).map(function(u){var o=identityOrg(u.org),row=[esc(u.name),esc(u.login),esc(o.name),esc(identityTypes[o.type]),esc(identityCreatedAt(u.createdAt)),'<button class="link-button" data-action="identityRole:'+u.role+'">'+esc(roles[u.role].label)+'</button>',badge(u.active?'启用':'停用',u.active?'success':'')];if(identityWritable())row.push(identityWritable(u)?button('维护账号','identityEdit:'+u.id,'link-button'):'只读');return row;});}
  function identityUsers(orgId){var headers=['姓名','登录账号','所属组织','组织类型','创建时间','角色与权限','状态'];if(identityWritable())headers.push('操作');return '<section class="identity-workspace">'+header(orgId?'组织成员':'用户与权限','账号归属一个组织、使用一个固定角色；组织决定数据归属，角色决定业务能力。',button('刷新','identityRefresh','secondary-button')+(identityWritable()?button('添加成员','identityAdd'+(orgId?':'+orgId:''),'primary-button'):''))+(!identityWritable()?'<p class="identity-alert">当前角色只读，可查看账号和权限说明</p>':'')+'<div class="identity-card">'+identityFilter(false,orgId)+identityPaged(headers,identityMemberRows(identity.users.filter(function(u){var q=identity.query;return (!q.q||(u.name+' '+u.login).toLowerCase().indexOf(q.q.toLowerCase())>=0)&&(!(orgId||q.org)||u.org===(orgId||q.org))&&(!q.role||u.role===q.role)&&(!q.status||u.active===(q.status==='enabled'));})))+'</div></section>';}
  function identityDrawer(){var o=identityOrg(identity.detail);if(!o)return '';var admins=identityAdmins(o.id);return '<div class="identity-drawer-mask" data-action="identityBackdrop"><section class="identity-drawer" role="dialog" aria-modal="true" aria-label="组织详情"><header><h2>组织详情</h2><button aria-label="关闭组织详情" data-action="identityBack">×</button></header><div class="identity-drawer-body"><dl class="identity-descriptions"><dt>组织编号</dt><dd>'+esc(o.id)+'</dd><dt>状态</dt><dd>启用</dd><dt>组织名称</dt><dd>'+esc(o.name)+'</dd><dt>组织类型</dt><dd>'+identityTypes[o.type]+'</dd></dl>'+(o.site?'<p>'+esc(o.site)+' · '+esc(o.area)+' · '+esc(o.address)+'</p>':'')+'<p>启用管理员：'+(admins.map(function(u){return esc(u.name)+'（'+esc(u.login)+'）';}).join('、')||'暂无')+'</p>'+(!admins.length?'<p class="identity-alert warning">此组织缺少启用管理员，请补齐管理员</p>'+(identityWritable()&&(o.type!=='platform'||state.role==='platform_admin')?button('补齐管理员','identityAdmin:'+o.id,'primary-button'):''):'')+identityUsers(o.id)+'</div></section></div>';}
  function identityRender(section){
    if(section==='用户与权限')return identityUsers();
    if(section==='客户与组织'){var q=identity.orgQuery||{},rows=identity.orgs.filter(function(o){return (!q.q||o.name.toLowerCase().indexOf(q.q.toLowerCase())>=0)&&(!q.type||o.type===q.type);}).sort(function(a,b){for(var key of ['type','name','id']){if(a[key]!==b[key])return a[key]<b[key]?-1:1;}return 0;}).map(function(o){return [button(o.name,'identityOrg:'+o.id,'link-button')+'<span class="cell-sub">'+esc(o.id)+'</span>',identityTypes[o.type],badge('启用',''),button('详情与成员','identityOrg:'+o.id,'link-button')];});return '<section class="identity-workspace">'+header(section,'集中管理酒店、洗涤厂及其管理员和成员。新组织建档时同步创建首个管理员。',button('刷新','identityRefresh','secondary-button')+(identityWritable()?button('新建组织','identityHotel','primary-button'):''))+'<div class="identity-card">'+identityFilter(true)+(rows.length?table(['组织名称 / 编号','组织类型','状态','操作'],rows).replace(/<div class="pagination">[\s\S]*$/,''):'<div class="identity-empty">暂无组织</div>')+'</div></section>'+identityDrawer();}
  ...
  function identityDrawMember(){var d=identity.draft,u=d.id?identityUser(d.id):null,valid=identityAssignable(d.org,u),restriction=identityRestriction(u);d.mode=d.mode||'edit';if(d.driverOnly)valid=valid.filter(function(r){return r==='driver';});if(d.adminOnly)valid=valid.filter(function(r){return r===identityOrg(d.org).type+'_admin';});if(valid.indexOf(d.role)<0)d.role=d.adminOnly||d.driverOnly?valid[0]||'':'';var orgField=d.locked?identityInput('所属组织','orgLabel',identityOrg(d.org).name,'disabled'):'<label class="form-row"><span>所属组织</span><input data-identity-field="orgSearch" list="identityOrgOptions" aria-label="所属组织" value="'+esc((identityOrg(d.org)||{}).name||'')+'" placeholder="搜索组织名称"><datalist id="identityOrgOptions">'+identity.orgs.filter(function(o){return !d.driverOnly||o.type==='platform'||o.type==='factory';}).map(function(o){return '<option value="'+esc(o.name)+'"></option>';}).join('')+'</datalist></label>';
    ...
    if(d.mode==='edit'){fields+=orgField+identityInput('登录账号','login',d.login,u?'disabled':'autocomplete="off"')+identityInput('姓名','name',d.name,'maxlength="100"')+identitySelect('角色','role',valid.map(function(r){return{id:r,name:roles[r].label};}),d.role,d.adminOnly||d.driverOnly?undefined:'请选择角色')+identityRoleInfo(d.role);
      ...
      if(!u&&d.role==='driver')fields+='<label class="form-row"><span>负责酒店（业务分配）</span><input data-identity-field="hotelSearch" list="identityHotelOptions" aria-label="负责酒店（业务分配）" placeholder="搜索负责酒店" value="'+esc((identityOrg(d.hotels[0])||{}).name||'')+'"><datalist id="identityHotelOptions">'+identity.orgs.filter(function(o){return o.type==='hotel';}).map(function(o){return '<option value="'+esc(o.name)+'"></option>';}).join('')+'</datalist></label>'+identityInput('负责区域','area',d.area);
    }
  ...
  function identityDrawOrg(){var d=identity.draft,steps='<ol class="identity-steps">'+['组织资料','首个管理员','确认创建'].map(function(label,i){return '<li class="'+(d.step===i+1?'active':'')+'">'+(i+1)+' · '+label+'</li>';}).join('')+'</ol>',body='';if(d.step===1)body=identitySelect('组织类型','type',[{id:'hotel',name:'酒店'},{id:'factory',name:'洗涤厂'}],d.type)+identityInput('组织名称','name',d.name)+identityInput('站点名称','site',d.site)+identityInput('所在区域','area',d.area)+identityInput('详细地址','address',d.address)+(d.type==='hotel'?'<label class="form-row"><span>楼层（逗号或换行分隔）</span><textarea data-identity-field="floors" aria-label="楼层（逗号或换行分隔）">'+esc(d.floors)+'</textarea></label>':'');if(d.step===2)body='<p class="identity-alert">创建'+(d.type==='hotel'?'酒店':'工厂')+'管理员，组织与管理员一次保存</p>'+identityInput('管理员姓名','adminName',d.adminName)+identityInput('管理员登录账号','login',d.login)+identityPasswords('初始密码');if(d.step===3)body='<dl class="identity-descriptions single"><dt>组织</dt><dd>'+esc(d.name)+'（'+identityTypes[d.type]+'）</dd><dt>站点</dt><dd>'+esc(d.site)+'</dd><dt>地址</dt><dd>'+esc(d.area)+' · '+esc(d.address)+'</dd>'+(d.type==='hotel'?'<dt>楼层</dt><dd>'+esc(d.floors)+'</dd>':'')+'<dt>管理员</dt><dd>'+esc(d.adminName)+' · '+esc(d.login)+'</dd></dl>';modalShell('新建组织','',steps+'<div class="identity-form">'+body+'</div><p class="identity-error" role="alert"></p>',button('取消','closeModal','secondary-button')+(d.step>1?button('上一步','identityOrgPrev','secondary-button'):'')+button(d.step===3?'确认创建':'下一步',d.step===3?'identityOrgCommit':'identityOrgNext','primary-button'));modal.querySelector('.modal-card').classList.add('identity-dialog','organization-dialog');if(d.step===2){modal.querySelector('[data-create-password]').value=d.password;modal.querySelector('[data-confirm-password]').value=d.confirmation;}}
  ...
  function identityHandle(action){
    ...
    if(name==='identityOrg'){identity.detail=id;identity.queryDraft=null;identity.query={q:'',org:'',role:'',status:'',type:''};identity.page=1;state.section='客户与组织';render();}
    if(name==='identityRefresh'){render();}if(name==='identityPage'){identity.page=Number(id);render();}if(name==='identityBack'||name==='identityBackdrop'){identity.detail='';identity.query={q:'',org:'',role:'',status:'',type:''};identity.queryDraft=null;identity.page=1;render();}
    if(name==='identityPrev'||name==='identityNext'){identity.page+=name==='identityNext'?1:-1;render();}
    if(name==='identitySearch'||name==='identityReset'){var scope=identity.detail?root.querySelector('.identity-drawer .identity-filter'):root.querySelector('.identity-filter'),query={q:'',org:'',role:'',status:'',type:''};if(name==='identitySearch'&&scope)scope.querySelectorAll('[data-identity-field]').forEach(function(el){if(el.dataset.identityField==='orgSearch'){var found=identity.orgs.find(function(o){return o.name===el.value;});query.org=found?found.id:el.value?'unknown':'';}else query[el.dataset.identityField]=el.value.trim();});if(scope&&scope.dataset.filterScope==='organizations'){identity.orgQuery=query;identity.orgQueryDraft=null;}else{identity.query=query;identity.queryDraft=null;}identity.page=1;render();}
  ...
  function setRole(key){if(!roles[key])return;closeModal();identity.detail='';identity.queryDraft=null;identity.page=1;identity.query={q:'',org:'',role:'',status:'',type:''};state.role=key;state.section=roles[key].nav[0];state.view='normal';viewSelect.value='normal';render();}
  ...
  function handleClick(event){
    ...
    var section=event.target.closest('[data-section]');if(section){state.section=section.dataset.section;identity.detail='';identity.queryDraft=null;identity.page=1;identity.query={q:'',org:'',role:'',status:'',type:''};render();return;}
